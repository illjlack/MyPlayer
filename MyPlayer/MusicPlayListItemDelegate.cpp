...
// 构造函数
MusicPlayListItemDelegate::MusicPlayListItemDelegate(QObject* parent)
...
// 自定义绘制方法
void MusicPlayListItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    qDebug() << "MusicPlayListItemDelegate:绘制===============";
...
// 自定义尺寸提示方法
QSize MusicPlayListItemDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    return QSize(option.rect.width(), 60); // 设置高度为60像素
}

// 图片下载完成后的槽函数
void MusicPlayListItemDelegate::onImageDownloaded(const QString& localPath, const QString& id, const QModelIndex& index) const
{
    qDebug() << "MusicPlayListItemDelegate: 下载图片完成 - 路径:" << localPath << "ID:" << id;
    ...
    if (index.isValid()) {
        ...
        if (model) {
            ...
            qDebug() << "MusicPlayListItemDelegate: 更新图片完成 - 索引:" << index;
        }
        else {
            qDebug() << "MusicPlayListItemDelegate: 获取模型失败";
        }
    }
    else {
        qDebug() << "MusicPlayListItemDelegate: 获取索引无效";
    }
}
